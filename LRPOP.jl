...
export  create_graph, clique_decomposition, variables_graph_xt, node_to_var,
        ...
        compute_lifting_bounds, add_ball_localizers_on_t!,
...
### Compute a tight bound on the absolute value of a univariate polynomial on [-ρ, ρ] using grid sampling
function tight_univariate_bound(cs::Vector{Float64}, ρ::Float64)
    grid = range(-ρ, ρ, length=200)
    max_val = maximum(abs(eval_unipoly(cs, x)) for x in grid)
    # Add a small safety margin (0.1%) to ensure it is a valid upper bound
    return max(max_val * 1.001, 1e-12)
end

### Compute bounds B_{l,i} for the lifting variables t_{l,i} recursively
function compute_lifting_bounds(
    R::Int, n::Int, alpha::Vector{Float64}, f_coeffs_R::Vector{Vector{Vector{Float64}}}
)
    B = [zeros(Float64, n) for _ in 1:R]
    for l in 1:R
        # For i = 1
        ρ1 = sqrt(alpha[1])
        sup1 = supnorm_poly_on_box_hilo(f_coeffs_R[l][1], ρ1)
        B[l][1] = sup1
        
        # For i = 2:n
        for i in 2:n
            ρi = sqrt(alpha[i])
            supi = supnorm_poly_on_box_hilo(f_coeffs_R[l][i], ρi)
            B[l][i] = B[l][i-1] * supi
        end
    end
    return B
...
# Add localizing constraint on t_{l,i} in the smallest clique possible
function add_ball_localizers_on_t!(
    model::Model,
    bag_y::Vector{<:Any},
    bag_vars::Vector{Vector{Symbol}},
    r::Int,
    t::Vector{Vector{Symbol}},
    ...
    for l in 1:R
        for i in 1:n
            tli = t[l][i]
            k = find_smallest_clique_with(bag_vars, [tli])
            vars = bag_vars[k]; y = bag_y[k]
            
            # Set the inequality B_{l,i}^2 - t_{l,i}^2 ≥ 0
            g = Tuple{Float64,NTuple{length(vars),Int}}[
                ( B[l][i]^2, exp_from_mon(vars, Dict{Symbol,Int}())),
                (-1.0,       exp_from_mon(vars, Dict(tli=>2)))
            ]
            add_localizing_psd!(model, y, vars, g, r)
        end
    end
...
function solve_LRPOP(R::Int, n::Int, d::Int,
    ...
    if add_t_bounds
        B_lifting = compute_lifting_bounds(R, n, alpha, f_coeffs_R)
        add_ball_localizers_on_t!(model, bag_y, clique_vars, r, t, B_lifting)
    end
